import {DataProvider, usePlasmicCanvasContext, useSelector} from "@plasmicapp/host";
...
function useTabsData({initialKey}: {initialKey?: string}) {
  ...
  const [bbox, setBbox] = useState<{left: number, width: number} | undefined>(undefined);
  return {
    tabKey,
    bbox,
    setTabKey,
...
export function TabsContainer({children, initialKey, previewKey}: TabsProviderProps) {
  return <TabsProvider initialKey={initialKey}><Helper previewKey={previewKey}>{children}</Helper></TabsProvider>
}

function Helper({children, previewKey}: {  previewKey?: string;
  children?: ReactNode}) {
  const {tabKey} = useTabsContext();
  const inEditor = usePlasmicCanvasContext();
  const effectiveKey = inEditor ? previewKey || tabKey : tabKey;
  return <DataProvider name={'currentTabKey'}
                data={effectiveKey}>{children}</DataProvider>
}
...
export function TabUnderline({className}: TabUnderlineProps) {
  ...
  console.log(bbox);
  return(
    bbox ? <div className={className} style={{...JSON.parse(JSON.stringify(bbox)), top: undefined, bottom: 0, position: 'absolute', transition: '.4s ease all'}}>
    </div> : null)
}
